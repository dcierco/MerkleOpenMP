...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel for schedule(static) reduction(+:local_validation_errors, local_execution_errors)
    for (int i = 0; i < num_transactions; i++) {
    ...
    num_threads_used = omp_get_max_threads(); // Fora da região, pega o máximo configurado
    ...
    if (execution_errors > 0) {
         printf("Execução Paralela com %d threads concluída com ERROS DE EXECUÇÃO.\n", num_threads_used);
    } else if (validation_errors > 0) {
         printf("Execução Paralela com %d threads concluída. %d FALHAS de validação encontradas.\n", num_threads_used, validation_errors);
    } else {
         printf("Execução Paralela com %d threads concluída com SUCESSO. Todas as %d transações são válidas.\n", num_threads_used, num_transactions);
    }
    printf("Tempo de execução da validação paralela: %f segundos\n", time_spent);
    printf("------------------------------------------------------------------\n");
